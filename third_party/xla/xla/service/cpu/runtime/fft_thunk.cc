...
FftThunk::FftThunk(Info thunk_info, const DebugOptions& debug_options,
    ...
    : Thunk(Kind::kFft, thunk_info),
      ...
      double_precision_(input_shape.element_type() == F64 ||
...
absl::StatusOr<std::unique_ptr<FftThunk>> FftThunk::Create(
  ...
  return absl::WrapUnique(new FftThunk(thunk_info, debug_options, fft_type,
                                       fft_length, input_buffer, output_buffer,
...
tsl::AsyncValueRef<Thunk::ExecuteEvent> FftThunk::Execute(
  ...
  // TODO(heinsaar): Remove the checks below once equivalent checks are done in
  // ThunkEmitter::EmitFftThunk() by calling
  // ThunkEmitter::ElementTypesSameAndSupported() once it is available.
  // Check that input and output have the same element type.
  TF_RET_CHECK(input_shape_.element_type() == output_shape_.element_type());
  TF_RET_CHECK(input_shape_.element_type() == F32 ||
               input_shape_.element_type() == F64 ||
               input_shape_.element_type() == C64 ||
               input_shape_.element_type() == C128);
  ...
  if (debug_options_.xla_cpu_multi_thread_eigen()) {
    ...
    __xla_cpu_runtime_DuccSingleThreadedFft(
        ...
        double_precision_, fft_rank, operand_shape_flat.data(),
        fft_length_.data());
  }
