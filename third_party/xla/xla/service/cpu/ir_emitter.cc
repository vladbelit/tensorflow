...
absl::Status IrEmitter::HandleCustomCall(HloInstruction* custom_call) {
  ...
  if (custom_call->shape().IsTuple()) {
    ...
    if (is_typed_ffi) {
      // Flatten nested tuples in the output
      TF_RETURN_IF_ERROR(ShapeUtil::ForEachLeafShapeWithStatus(
          custom_call->shape(),
          [&](const Shape& shape, const ShapeIndex& index) {
            TF_ASSIGN_OR_RETURN(BufferAllocation::Slice slice,
                                assignment_.GetUniqueSlice(custom_call, index));
            base_ptrs.push_back(EmitBufferPointer(slice, shape));
            return absl::OkStatus();
          }));
      ...
      for (int i = 0; i < ShapeUtil::TupleElementCount(custom_call->shape());
        ...
        base_ptrs.push_back(EmitBufferPointer(slice, elem_shape));
      }
    }
...
llvm::Value* IrEmitter::EmitCallToFfi(HloCustomCallInstruction* custom_call,
  ...
  std::vector<llvm::Value*> arguments = {
      ...
      operand_types_encoded.alloca,             // operand_types
      b_.getInt64(operand_types_encoded.size),  // operand_count
      operand_shapes_encoded.alloca,            // operand_dims
      result_types_encoded.alloca,              // result_types
      b_.getInt64(result_types_encoded.size),   // result_count
      result_shapes_encoded.alloca,             // result_dims
  };
