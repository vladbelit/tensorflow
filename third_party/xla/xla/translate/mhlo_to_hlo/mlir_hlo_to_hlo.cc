...
// `shardings`.
xla::XlaOp MaybeCreateTuple(
    xla::XlaBuilder* builder, llvm::ArrayRef<xla::XlaOp> elements,
    llvm::ArrayRef<std::optional<xla::OpSharding>> shardings) {
  if (elements.size() == 1) {
    return elements[0];
  }
...
llvm::SmallVector<std::optional<xla::OpSharding>> GetXlaOpShardings(
    llvm::ArrayRef<xla::XlaOp> xlaOps) {
  llvm::SmallVector<std::optional<xla::OpSharding>> shardings;
  shardings.reserve(xlaOps.size());
  for (const xla::XlaOp& xlaOp : xlaOps) {
    auto sharding = xlaOp.builder()->GetOpSharding(xlaOp);
...
LogicalResult ExportXlaOp(IfOp op, OpLoweringContext ctx) {
  ...
  xla::XlaOp true_arg =
      MaybeCreateTuple(ctx.builder, true_args, true_arg_shardings);
  ...
  xla::XlaOp false_arg =
      MaybeCreateTuple(ctx.builder, false_args, false_arg_shardings);
...
LogicalResult ExportXlaOp(CaseOp op, OpLoweringContext ctx) {
  ...
  for (unsigned i = 0; i < branches.size(); ++i) {
    ...
    branch_operands[i] = MaybeCreateTuple(ctx.builder, args, arg_shardings);
